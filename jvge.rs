...
//Whether Validation Layer should be enabled
const VALIDATION_ENABLE: bool = cfg!(debug_assertions);
...
fn main() -> Result<()> {
    ...
    event_loop.run(move |event, _, control_flow| {
            *control_flow = ControlFlow::Poll;
            match event {
                //Render frame if app is not being destroyed.
...
impl App {
    ...
    // Render a frame of the vulkan app
    unsafe fn render(&mut self, window: &Window) -> Result <()> {
        Ok(())
    }

    //Destroys out Vulkan app.
...
unsafe fn create_instance(window: &Window, entry: &Entry, data: &mut AppData) -> Result<Instance> {
    ...
    let application_info = vk::ApplicationInfo::builder()
        .application_name(b"jvge - Vulkanalia\0")
    ...
    if VALIDATION_ENABLE && !available_layers.contains(&VALIDATION_LAYER) {
        return Err(anyhow!("Validation layer requested but not supported."));
    }

    let layers = if VALIDATION_ENABLE {
...
unsafe fn check_physical_device(instance: &Instance, data: &AppData, physical_device: vk::PhysicalDevice) ->
                                Result<()> {
